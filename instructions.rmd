...
AIC and BIC are statistics that tells us the prediction error, which allows us to make conclusions on the quality of the data. by comparing your different models, you should be able to identify which datasets best fit the linear models by studying these two values. Typically, lower values mean the model fits the data better.
```{r}
Fglm = glm(X ~ x + y, data=F) #This forms the model as a query to the correlative relationship of X to x and y
